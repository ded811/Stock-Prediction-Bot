...
freq_entry = ttk.Combobox(window, values=['1H', '12h', '1D', '1W', '1M', '3M'])
freq_entry.grid(row=2, column=1)
freq_entry.set('1D')

tk.Label(window, text="Prediction Points: ").grid(row=3)
...
def get_prediction():
    ...
    fig.update_layout(height=1200, title=f"{ticker} Stock Prediction")
    fig.show()
